...
filedatetime = datetime.datetime.today()
filedatetime_string = filedatetime.strftime('%Y%m%d%H%M%S%M')
outputfile = localoutputfolder + '\\barclays upload ' + fileextension[(-1)*(len(fileextension)-1):] + ' returnsmonthlypriceandmarketvalue ' + filedatetime_string + '.csv'
uploadfile = etluploadfolder + '\\barclays upload ' + fileextension[(-1)*(len(fileextension)-1):] + ' returnsmonthlypriceandmarketvalue ' + filedatetime_string + '.csv'
...
for filename in files:
    ...
    if thisfileextension == fileextension:
            ...
            with open(localunprocessedcsvpathname, 'r') as f:
                ...
                for row in reader:
                    ...
                    if checkingon == 1 and IndexOfString > 0 and len(row) > IndexOfString:
                        #print 'ok on',row[1]
                        #print row
                        mymtdreturn = row[IndexOfString]
                        
                        onlydigits = mymtdreturn.replace(' ','').replace('.','').replace('-','')
                        if onlydigits.isdigit():
                            #print row[0],row[1],row[2],mymtdreturn
                            mydict[len(mydict)] = {'source':'BARCLAYS','measure':datacolumntitle,'product':fileextension,'id':row[0].strip(),'name':row[1],'period':row[2],'datavalue':mymtdreturn,'filename':filename}
        
        # #####################
        # Do not enable this for muni, must 3 processes on files for 3 measures (MTDTot,MktVal,Price)
        ...
        if os.path.exists(localprocessedcsvpathname):
            ...
            #zzzzzzz
        print 'attempting to move:',localunprocessedcsvpathname
        moveFile(localunprocessedcsvpathname, localprocessedfolder)

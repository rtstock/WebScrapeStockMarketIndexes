...
localprocessedfolder = 'C:\\Batches\\AutomationProjects\\Investment Strategy\\ETL\Downloads\\Unprocessed\\barclays\\Ready for daily processing'
                ...
                if 'MTD Total' in str(row):
                    ...
                    if len(str(row)) > mycharindex_end:
                        ...
                        if mymtdreturn.strip().replace('-','').replace('.','').isdigit() == True:
                            mydict[len(mydict)] = {'source':'barclays','category':'hyd','mysection1':mysection1,'mysection2':mysection2,'issue':issuename.strip(),'mtdtotalreturn':mymtdreturn.strip(),'filename':filename}

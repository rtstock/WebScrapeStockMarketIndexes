class perform:
    ...
    def execute(self,localunprocessedcsvpathname = ''):
        ...
        print d
        mtdreturn = float(d[0]['mtdreturn'])
        dayofmonth = int(d[0]['dayofmonth'])
        myfilename = d[0]['filename']
        lstx = self.calcdaily(mtdreturn,dayofmonth)
        filenamedate8 = os.path.basename(myfilename).split('.')[0]
        dictofdate8s = self.getdate8dictionary(filenamedate8,dayofmonth)
        print dictofdate8s
        resultlst = []
        for k,d8 in dictofdate8s.items():
                resultlst.append({'date8':d8,'averagedailyreturn':lstx[k]})
        mygeomean = self.calcgeomean(resultlst)
        print 'mygeomean',mygeomean
        return resultlst
...
if __name__ == "__main__":
    ...
    if 1 == 1:
        ...
        x = o.execute(filename)
        print x
    if 1 == 2:
        import os
        o = perform()
        filename = os.path.join('C:\\Batches\\AutomationProjects\\Investment Strategy\\ETL\\Downloads\\Unprocessed\\barclays\\Ready for daily processing\\','20170314.agg')
        mydict = o.getrawreturnvaluedict(filename)

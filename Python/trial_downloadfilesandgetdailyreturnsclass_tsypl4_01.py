class perform:
    ...
    def get_treasury_oneyear_constant_maturity_history_daily(self):
        ...
        #y = 2/0
        ls1 = self.calcdaily(float(latest_non_zero_date_and_value[1]),int(latest_non_zero_date_and_value[0][6:]))
        ls2 = self.calcdaily(float(4.0),int(latest_non_zero_date_and_value[0][6:]))
        
        refdate = datetime.date(myyear, mymonth, daysinmonth)
        dictofdate8s = self.getdictofdate8sfromrefdate(refdate,daysinmonth)
        lstfinal = []
        for k,v in dictofdate8s.items():
            dictcurr = {'date8':v,'dailyreturn':ls1[0]+ls2[0] }
            #print dictcurr
            lstfinal.append(dictcurr)
        #FFFFFF
        #print lstfinal

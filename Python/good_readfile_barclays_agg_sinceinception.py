        ...
        with open(localunprocessedcsvpathname, 'r') as f:
            ...
            for row in reader:
                ...
                if '    Since' in str(row):
                    ...
                    mynugget = str(row)[mycharindex_start:mycharindex_end]
                    onlydigits = mynugget.replace(' ','').replace('.','').replace('-','')
                    ...
                    if len(str(row)) > mycharindex_end:
                        #print str(row)[2:21],str(row)[22:31],str(row)[32:38],mynugget,filename,'|||||',row[0], 'xxxxxxxx', str(row)[32:39]
                        issuename = str(row)[2:len('  Financial Institutio')]
                        issuename = issuename.replace('[','')
                        issuename = issuename.replace(']','')
                        issuename = issuename.replace('\'','')
                        #if not issuename[:2] == '  ':
                        if mynugget.strip().replace('-','').replace('.','').isdigit() == True:
                            mydict[len(mydict)] = {'source':'barclays','category':'agg','mysection1':mysection1,'mysection2':mysection2,'issue':issuename.strip(),'mynugget':mynugget.strip(),'filename':filename}

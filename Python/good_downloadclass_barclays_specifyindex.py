...
class perform:
    ...
    def execute(self, download_last_x_days, specificextension, asofdate):
        #################################################################################
...
if __name__ == "__main__":
    import sys
    import datetime
    anchorday = datetime.datetime.today()

    try:
        import os
        o = perform()
        if len(sys.argv) > 1:
            exec_result = o.execute(sys.argv[0],sys.argv[1],anchorday)
        else:
            exec_result = o.execute(-3000,'.agg.gz',anchorday)
